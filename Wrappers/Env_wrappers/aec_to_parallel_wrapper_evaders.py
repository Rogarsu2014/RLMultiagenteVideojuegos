...
class aec_to_parallel_wrapper_evaders(aec_to_parallel_wrapper):
    ...
    def reset(self, seed=None, options=None):
        ...
        observations_evaders = {
            ...
            for evader in self.aec_env.evaders
            #if not (self.aec_env.terminations[agent] or self.aec_env.truncations[agent])
        }
    ...
    def step(self, actions):
        ...
        for evader in self.aec_env.evaders:
            if evader != self.aec_env.agent_selection:
                raise AssertionError(
                    f"expected agent {evader} got agent {self.aec_env.agent_selection}, Parallel environment wrapper expects agents to step in a cycle. Error evader manual"
                )
            #No sabemos si esto se usa o no #obs, rew, termination, truncation, info = self.aec_env.last()
            self.aec_env.step(actions[evader])
            for evader in self.aec_env.evaders:
                rewards_evaders[evader] += self.aec_env.rewards_evaders[evader]
        ...
        observations_evaders = {
            evader: self.aec_env.observe(evader) for evader in self.aec_env.evaders
        }
        ...
        return observations, rewards, terminations, truncations, infos, observations_evaders, rewards_evaders
